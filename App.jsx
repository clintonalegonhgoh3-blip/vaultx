...
function Toast({ message, type = "success", onClose }) {
  ...
  const bg = type === "success" ? "bg-emerald-500" : type === "error" ? "bg-red-500" : type === "warning" ? "bg-yellow-500" : "bg-blue-500";
  return (
...
function CreateVaultPage({ onCreate }) {
  ...
  const [phraseConfirm, setPhraseConfirm] = useState(Array(6).fill(""));
  ...
  const handleConfirmPhrase = async () => {
    ...
    try {
      ...
      DB.save({
        encrypted, salt: Array.from(salt), version: 1,
        ...
        <div className="rounded-2xl p-8 border" style={{ background: "rgba(255,255,255,0.03)", borderColor: "rgba(255,255,255,0.07)" }}>
          <div className="flex items-start gap-3 mb-5 p-4 rounded-xl" style={{ background: "rgba(234,179,8,0.1)", border: "1px solid rgba(234,179,8,0.3)" }}>
            ...
            <p className="text-yellow-300 text-xs leading-relaxed">
              <strong>Notez ces 6 mots dans l'ordre sur papier.</strong><br />
          ...
          <button onClick={() => { setPhraseConfirm(Array(6).fill("")); setStep(3); }}
  ...
  if (step === 3) return (
    <div className="min-h-screen flex items-center justify-center p-4" style={{ background: "linear-gradient(135deg, #0a0a0f 0%, #0d1117 50%, #0a0f1a 100%)" }}>
      <div className="w-full max-w-lg">
        ...
        <div className="rounded-2xl p-8 border" style={{ background: "rgba(255,255,255,0.03)", borderColor: "rgba(255,255,255,0.07)" }}>
          <p className="text-white/60 text-sm mb-4">Entrez vos 6 mots dans l'ordre pour confirmer.</p>
...
function RecoveryPage({ onBack, onRecovered }) {
  ...
  const handleVerifyPhrase = async () => {
    ...
    try {
      ...
      // ✅ Phrase correcte — déchiffrer et exporter automatiquement
      const phraseSalt = new Uint8Array(stored.recovery.phraseSalt);
      const phraseKey = await Crypto.deriveKey(entered.join(" "), phraseSalt);
      ...
      // Export automatique du coffre en clair (JSON lisible)
      const exportData = {
        exportedAt: new Date().toISOString(),
        application: "VaultX",
        note: "Export automatique lors de la récupération du mot de passe maître",
      ...
      a.href = url;
      a.download = "vaultx-recovery-export-" + new Date().toISOString().split("T")[0] + ".json";
      a.click();
      setTimeout(() => URL.revokeObjectURL(url), 5000);
      // Stocker l\'ancienne clé pour rechiffrement après
      setOldKey(oldKey);
      setVaultData(vault);
      setStep(2);
    } catch { setError("Erreur de vérification. Vérifiez vos 6 mots."); }
    setLoading(false);
  };
    ...
    <div className="min-h-screen flex items-center justify-center p-4" style={{ background: "linear-gradient(135deg, #0a0a0f 0%, #0d1117 50%, #0a0f1a 100%)" }}>
      <div className="w-full max-w-lg">
        ...
        <div className="rounded-2xl p-8 border" style={{ background: "rgba(255,255,255,0.03)", borderColor: "rgba(255,255,255,0.07)" }}>
          {step === 1 ? (
            <>
              <p className="text-white/60 text-sm mb-4">Entrez vos 6 mots de récupération dans l'ordre exact.</p>
              <div className="grid grid-cols-2 gap-2 mb-4">
                {words.map((w, i) => (
                  <div key={i} className="flex items-center gap-1">
                    <span className="text-white/30 text-xs w-4 flex-shrink-0">{i + 1}.</span>
            ...
            <>
              <div className="flex items-center gap-2 mb-3 p-3 rounded-xl" style={{ background: "rgba(34,197,94,0.1)", border: "1px solid rgba(34,197,94,0.3)" }}>
                <Icon.Check /><span className="text-emerald-400 text-sm font-medium">Phrase vérifiée ✓</span>
              </div>
              <div className="flex items-start gap-2 mb-4 p-3 rounded-xl" style={{ background: "rgba(234,179,8,0.1)", border: "1px solid rgba(234,179,8,0.3)" }}>
                <span className="text-yellow-400 flex-shrink-0">📥</span>
                <span className="text-yellow-300 text-xs leading-relaxed">Vos mots de passe ont été <strong>exportés automatiquement</strong> en fichier JSON dans vos téléchargements. Conservez ce fichier en lieu sûr.</span>
              </div>
        ...
        <div className="rounded-2xl p-8 border" style={{ background: "rgba(255,255,255,0.03)", borderColor: "rgba(255,255,255,0.07)" }}>
          <div className="space-y-4">
            ...
            <button onClick={onReset} className="w-full py-2 text-white/30 hover:text-red-400 text-xs transition-colors">
              Réinitialiser le coffre pour créer un nouveau(⚠️ Attention cette action supprime automatiquement toutes les données de l'ancien coffre !)
            </button>
          </div>
        </div>
      </div>
